...
<br>
    ...
    <div class="search-bar col-md-3">
      <form class="search-form d-flex align-items-center" method="GET" action="{{ route('user/bmr/data_bmr/cari_bmr') }}">
      <select class="form-control" name="id_rw" id="id_rw" required>
        ...
        <a href="{{ route('user/bmr/data_bmr') }}" class="btn btn-success btn-sm">Refresh</a>
      </form>
      </div><!-- End Search Bar -->
    <br>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td> 
                      ...
                      @elseif($item->verifikasi =='Terverifikasi')
                      <a href="{{ url('user/bmr/lihat_data_bmr/'.$item->id) }}" class="btn btn-info" style="font-size: 14px; color: white" ></i> Lihat Data</a>	
                      @else($item->verifikasi =='Ditolak')
                      <a href="{{ url('user/bmr/lihat_data_bmr/'.$item->id) }}" class="btn btn-info" style="font-size: 14px; color: white" ></i> Lihat Data</a>
                      @endif
                    ...
                    <td>
					  ...
					  @elseif($item->verifikasi =='Terverifikasi')
                      <a href="{{ url('user/bmr/surat_bmr/'.$item->id) }}" target="_blank" class="btn btn-primary btn-sm" style="font-size: 14px; color: white" ></i> Download</a>	
                      @else($item->verifikasi =='Ditolak')
                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <form action="{{ route('delete_bmr') }}" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('DELETE')}}
