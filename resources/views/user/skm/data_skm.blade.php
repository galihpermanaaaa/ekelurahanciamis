...
<main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Daftar Surat Keterangan Tidak Mampu</h1>
    </div><!-- End Page Title -->
...
<br>
    ...
    <div class="search-bar col-md-3">
      <form class="search-form d-flex align-items-center" method="GET" action="{{ route('user/skm/data_skm/cari_skm') }}">
      <select class="form-control" name="id_rw" id="id_rw" required>
        ...
        <a href="{{ route('user/skm/data_skm') }}" class="btn btn-success btn-sm">Refresh</a>
      </form>
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Daftar Pembuat SKM</h5>
              ...
              <table class="table datatable">
                ...
                <tbody>
                  ...
                  <tr>
                      ...
                      <td class="text-right">
                      @if($item->verifikasi =='Belum Diverifikasi')
                      <button type="button" class="btn btn-danger" href="#" data-bs-toggle="modal" data-bs-target="#hapus_skm" data-id="{{$item->id}}" data-nikk="{{$item->nik}}"><i class="bi bi-trash"></i> Hapus</button>
                      @elseif($item->verifikasi =='Terverifikasi')
                    ...
                    <td> 
                     ...
                     <p>Harus Diverifikasi terlebih dahulu</p>
                      @elseif($item->verifikasi =='Terverifikasi')
                      <a href="{{ url('user/skm/lihat_data_skm/'.$item->id) }}" class="btn btn-info" style="font-size: 14px; color: white" ></i> Lihat Data</a>	
                      @else($item->verifikasi =='Ditolak')
                      <a href="{{ url('user/skm/lihat_data_skm/'.$item->id) }}" class="btn btn-info" style="font-size: 14px; color: white" ></i> Lihat Data</a>	
                      @endif
              ...
              <span class="badge bg-success">Jumlah Surat Keterangan Tidak Mampu Terverifikasi: {{$count_terverifikasi}}</span><br>
              <span class="badge bg-danger">Jumlah Surat Keterangan Tidak Mampu Ditolak: {{$count_ditolak}}</span>
    ...
    <div id="hapus_skm" class="modal custom-modal fade" role="dialog">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <form action="{{ route('delete_skm') }}" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('DELETE')}}
...
$('#hapus_skm').on('show.bs.modal', function (event) {
